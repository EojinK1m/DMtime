...
def test_gallery_delete_without_authority(client, create_temp_account, create_temp_gallery):
    temp_account = create_temp_account()
...
def test_gallery_patch_correct(client, create_temp_account, create_temp_gallery):
    ...
    rv = client.patch(url+f'{temp_gallery.id}',
...
def test_gallery_patch_with_data_miss(client, create_temp_account, create_temp_gallery):
    ...
    rv_2 = client.patch(url + f'{temp_gallery.id}',
                        headers={'authorization': 'Bearer ' + admin_account.generate_access_token()},
...
def test_gallery_patch_to_already_exist_gallery_name(client, create_temp_account, create_temp_gallery):
    temp_account = create_temp_account()
    temp_gallery1, temp_gallery2 = create_temp_gallery(temp_account),\
                                   create_temp_gallery(temp_account)

    rv = client.patch(url+f'{temp_gallery1.id}',
                      headers={'authorization':'Bearer '+temp_account.generate_access_token()},
