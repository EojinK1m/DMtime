...
class UserService:
    ...
    def provide_user_info(username):
        ...
        return jsonify(
            {"msg": "query succeed", "user_info": user_schema.dump(find_user)}
        )
    ...
    def modify_user_info(username, data):
        ...
        new_profile_image_id = data.get(
            "profile_image_id", user.profile_image.id if user.profile_image else None
        )
        ...
        if not UserService.set_profile_image(user, new_profile_image_id):
            return (
                jsonify(
                    {
                        "msg": f"image not found, image {new_profile_image_id}is not exist"
                    }
                ),
                404,
            )
...
class AccountService:
    ...
    def provide_account_info():
        ...
        return (
            jsonify(account_info=account_schema.dump(account), msg="query succeed"),
            200,
        )
    ...
    def register_account(data):
        ...
        AccountService.send_verification_by_email(
            verification_code=verification_code, to_send_email=email
        )

        return (
            jsonify(
                message="register account temporally succeed, plz verify requested email"
            ),
            201,
        )
    ...
    def verify_email_verification_code(verification_code):
        ...
        register_data = AccountService.find_register_data_by_verification_code(
            verification_code
        )
    ...
    def store_account_user_information(register_data):
        try:
            ...
            new_user = UserModel(
                username=register_data.get("username"), account=new_account
            )
